...
// 两阶段最小二乘，三种命令
ivregress 2sls  any_prio (gdp_g = GPCP_g) i.ccode i.ccode#c.year, cl(ccode)
ivreghdfe any_prio (gdp_g = GPCP_g), absorb(i.ccode i.ccode#c.year) cl(ccode)
ivreg2 any_prio (gdp_g = GPCP_g) i.ccode i.ccode#c.year, cl(ccode)
...
// 两阶段最小二乘，三种命令
ivregress 2sls any_prio (gdp_g gdp_g_l = GPCP_g GPCP_g_l ) i.ccode i.ccode#c.year, cl(ccode)
ivreghdfe any_prio (gdp_g gdp_g_l = GPCP_g GPCP_g_l ), absorb(i.ccode i.ccode#c.year) cl(ccode)
ivreg2  any_prio (gdp_g gdp_g_l = GPCP_g GPCP_g_l ) i.ccode i.ccode#c.year, cl(ccode)
// 两阶段最小二乘，多余的(四个)工具变量
ivreghdfe any_prio (gdp_g gdp_g_l =GPCP_g GPCP_g_l NCEP_g NCEP_g_l), absorb(i.ccode i.ccode#c.year) cl(ccode)
** 有限信息极大似然
