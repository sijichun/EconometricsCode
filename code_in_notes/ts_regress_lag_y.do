...
outreg2 using ts_reg_lay_y.tex, replace /*
  */ adds(Breusch-Godfrey, `p', Long-run effects, `le')
reg gdp_growth L.gdp_growth L(0/1).einvq0105
estat bgodfrey, lags(2)
local p=r(p)[1,1]
local le=(_b[einvq0105]+_b[L.einvq0105]) ///
   /(1-_b[L.gdp_growth])
outreg2 using ts_reg_lay_y.tex, append /*
  */adds(Breusch-Godfrey, `p', Long-run effects, `le')
reg gdp_growth L(1/2).gdp_growth L(0/1).einvq0105
estat bgodfrey, lags(2)
local p=r(p)[1,1]
local le=(_b[einvq0105]+_b[L.einvq0105]) ///
   /(1-_b[L.gdp_growth]-_b[L2.gdp_growth])
outreg2 using ts_reg_lay_y.tex, append /*
  */ adds(Breusch-Godfrey, `p', Long-run effects, `le')
reg gdp_growth L(1/2).gdp_growth L(0/2).einvq0105
estat bgodfrey, lags(2)
local p=r(p)[1,1]
local le=(_b[einvq0105]+_b[L.einvq0105]+_b[L2.einvq0105]) ///
   /(1-_b[L.gdp_growth]-_b[L2.gdp_growth])
outreg2 using ts_reg_lay_y.tex, append /*
  */ adds(Breusch-Godfrey, `p', Long-run effects, `le')
